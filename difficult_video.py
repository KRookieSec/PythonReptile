    ...
    async with aiohttp.ClientSession() as session:
        async with aiofiles.open(f"越狱/越狱第一季第一集_second_m3u8.txt",mode="r",encoding="utf-8") as f:
            async for line in f:
                ...
                #创建任务
                task = asyncio.create_task(download_ts(url,linename,session))
                tasks.append(task)
...
async def dec_ts(name,key):
    aes = AES.new(key=key, IV=b"000000000000000",mode=AES.MODE_CBC)
    async with aiofiles.open(f"越狱/video/{name}",mode="rb") as f1,\
        aiofiles.open(f"越狱/video/temp_{name}",mode="wb") as f2:
...
async def aio_dec(key):
    ...
    async with aiofiles.open("越狱/越狱第一季第一集_second_m3u8.txt",mode="r",encoding="utf-8") as f:
        async for line in f:
            ...
            linename = line.split("/hls")[-1]
            #开始创建异步任务
            task = asyncio.create_task(dec_ts(linename,key))
        await asyncio.wait(tasks)
...
def is_key(name):
    tasks = []
    with open(f"越狱/video/{name}",mode="r",encoding="utf-8") as f:
        for line in f:
            #如果以https开头则舍弃
            ...
            line = line.strip()
            #提取密钥链接
            key_url = re.findall('(https?://[^\s]+)"',line)
            key = get_key(key_url)
            print(key)
